...
public class FileStreamThread extends Thread {
    ...
    FileStreamThread(Context mContext, String filePath, int sendSize, DataRaderListener listener) {
    ...
    public void run() {
        try {
            prepareData(sendSize);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    private void prepareData(int sendSize) throws IOException {
        initStream();
        byte[] block = new byte[sendSize];
        int dataLength;
        ...
        while (isKeepRunning) {
            ...
            byte[] packige = DFU025Util.getDataPackage(block, dataLength);
    ...
    private void initStream() {
        if (inputStream == null) {
            try {
                inputStream = DFU025Util.getInputStream(mContext, filePath);
